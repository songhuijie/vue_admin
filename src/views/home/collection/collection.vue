<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="分类" min-width="110px">
        <template slot-scope="{row}">
...
import { fetchList, createCollection, updateCollection, deleteCollection } from '@/api/home/collection/collection'
...
const TypeOptions = [
  { key: 1, display_name: '校史馆' },
  { key: 2, display_name: '人体科学博物馆' },
  { key: 3, display_name: '图书馆' }
]
...
export default {
  ...
  methods: {
      ...
      fetchList(this.listQuery).then(response => {
        response = this.$notify({
          ...
          data: {
            total: 3,
            items: [
              { 'id': 1, 'number': 'YW_abc12345678', 'type': 1, 'cover': 'https://axure-file.lanhuapp.com/75bb826c-df35-4a34-b9a5-86b4cff4543c__28e3bf0b1943ba0890f45e053338de22.png', 'created_at': '2020-04-30 11:11:11', 'updated_at': '2020-04-30 11:11:11', 'is_relation_rotation': 0, 'is_relation_memorabilia': 0, 'content': '内容' },
              { 'id': 2, 'number': 'YW_abc12345678', 'type': 2, 'cover': 'https://axure-file.lanhuapp.com/75bb826c-df35-4a34-b9a5-86b4cff4543c__28e3bf0b1943ba0890f45e053338de22.png', 'created_at': '2020-04-30 11:11:11', 'updated_at': '2020-04-30 11:11:11', 'is_relation_rotation': 1, 'is_relation_memorabilia': 1, 'content': '内容' },
              { 'id': 3, 'number': 'YW_abc12345678', 'type': 3, 'cover': '', 'created_at': '2020-04-30 11:11:11', 'updated_at': '2020-04-30 11:11:11', 'is_relation_rotation': 1, 'is_relation_memorabilia': 1, 'content': '内容' }
            ]
    ...
    handleChangeStatus(row, status, key) {
      ...
      updateCollection(row, row.id).then(() => {
        this.$notify({
          title: 'Success',
          message: '更新成功',
          type: 'success',
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        ...
        //   this.temp.author = 'vue-element-admin'
          ...
          createCollection(this.temp).then(() => {
    ...
    updatePicData() {
      ...
      this.$refs['dataFormPic'].validate((valid) => {
        if (valid) {
          ...
          updateContact(tempData, tempData.id).then(() => {
            this.$notify({
              title: 'Success',
              message: 'Update Successfully',
              type: 'success',
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          tempData.timestamp = +new Date(tempData.timestamp) // change Thu Nov 30 2017 16:41:05 GMT+0800 (CST) to 1512031311464
          updateCollection(tempData, tempData.id).then(() => {
            this.$notify({
              title: 'Success',
              message: 'Update Successfully',
              type: 'success',
